# 飞书机器人配置完整指南

本文档记录了飞书机器人所需的完整配置项，方便其他项目复用。

## 📋 配置清单

### 1. 创建应用

**访问地址**：https://open.feishu.cn/app

**步骤**：
1. 点击"创建企业自建应用"
2. 选择应用类型：企业自建应用
3. 填写应用信息：
   - 应用名称：Claude Stream Bot（或自定义名称）
   - 应用描述：Claude CLI 流式对话机器人
4. 创建后记录以下信息：
   - **App ID**：`cli_a9dc39c0c2b8dbc8`（示例）
   - **App Secret**：从"凭证与基础信息"页面获取

### 2. 配置权限

**访问地址**：https://open.feishu.cn/app/cli_a9dc39c0c2b8dbc8/auth

**必需权限**：

| 权限名称 | 权限ID | 用途 | 状态 |
|---------|--------|------|------|
| 获取与发送单聊、群组消息 | `im:message` | 接收和发送消息 | ✅ 必需 |
| 群聊中@消息的接收 | `im:message.group_at_msg` | 接收群聊@消息 | ✅ 必需 |
| 访问群聊信息 | `im:chat` | 访问群组信息 | ✅ 必需 |
| 创建与更新卡片 | `cardkit:card:write` | CardKit 流式更新 | ✅ 必需 |

**配置步骤**：
1. 访问权限管理页面
2. 搜索并启用上述4个权限
3. 点击"保存"
4. （重要）**发布版本**：权限修改后需要创建版本才能生效

### 3. 配置事件订阅

**访问地址**：https://open.feishu.cn/app/cli_a9dc39c0c2b8dbc8/event

**订阅方式**：使用长连接接收事件

**配置步骤**：

#### 步骤 1：选择订阅方式
1. 点击"订阅方式"按钮
2. 选择"**使用长连接接收事件**"
3. 点击"保存"

**说明**：
- 长连接模式无需公网域名或 Webhook
- 需要使用飞书官方 SDK 启动长连接客户端
- 适合本地开发和测试

#### 步骤 2：添加事件
1. 点击右上角"**添加事件**"按钮
2. 在搜索框中输入：`message` 或 `im.message`
3. 找到"**im.message.receive_v1**"（接收消息）
4. 勾选该事件
5. 点击"确认添加"
6. 等待页面刷新，确认事件已添加

**事件详情**：
- **事件名称**：im.message.receive_v1
- **事件类型**：消息
- **订阅类型**：应用身份订阅
- **所需权限**：`im:message`（已在步骤2中配置）
- **用途**：接收用户发送的消息（单聊和群聊）

#### 步骤 3：验证配置
确认页面显示：
- ✅ 订阅方式：长连接
- ✅ 已添加事件：im.message.receive_v1
- ✅ 不再显示"暂无数据"

### 4. 启动机器人

**前提条件**：
- ✅ 已完成应用配置
- ✅ 已配置权限
- ✅ 已配置事件订阅
- ✅ 飞书机器人已在测试群中

**启动命令**：
```bash
cd /path/to/feishu-bot
go run cmd/bot/main.go > /tmp/feishu-bot.log 2>&1 &
```

**验证连接**：
```bash
# 查看日志
tail -f /tmp/feishu-bot.log

# 应该看到
[Info] connected to wss://msg-frontier.feishu.cn/ws/v2
```

### 5. 测试机器人

**在测试群中测试**：
```
@机器人 你好
```

**预期结果**：
1. 群聊中创建新的卡片
2. 卡片显示"思考中..."
3. 卡片内容流式更新（打字机效果）
4. 机器人日志显示收到消息

**查看日志**：
```bash
# 实时查看日志
tail -f /tmp/feishu-bot.log

# 查看消息接收
grep "Message received" /tmp/feishu-bot.log

# 查看流式对话
grep "streaming chat" /tmp/feishu-bot.log
```

## 🔧 环境配置

### .env 文件

```bash
# 飞书应用配置
FEISHU_APP_ID=cli_a9dc39c0c2b8dbc8
FEISHU_APP_SECRET=your_app_secret_here

# 机器人配置（可选）
BOT_NAME=Claude Stream Bot
LOG_LEVEL=info

# 群聊是否必须@机器人（默认 false）
GROUP_REQUIRE_MENTION=false
```

### 应用配置文件

**configs/config.yaml**：
```yaml
app:
  id: "cli_a9dc39c0c2b8dbc8"
  name: "Claude Stream Bot"
  verification_token: ""

server:
  port: 8080

feishu:
  base_url: "https://open.feishu.cn"
```

## 📊 配置检查清单

部署新项目时，按以下顺序检查：

- [ ] **1. 应用创建**
  - [ ] 已创建企业自建应用
  - [ ] 已记录 App ID 和 App Secret

- [ ] **2. 权限配置**
  - [ ] `im:message` ✅
  - [ ] `im:message.group_at_msg` ✅
  - [ ] `im:chat` ✅
  - [ ] `cardkit:card:write` ✅
  - [ ] 已发布版本（权限生效）

- [ ] **3. 事件订阅**
  - [ ] 订阅方式：长连接 ✅
  - [ ] 已添加：im.message.receive_v1 ✅
  - [ ] 配置已保存 ✅

- [ ] **4. 代码配置**
  - [ ] .env 文件已配置
  - [ ] App ID 和 Secret 正确
  - [ ] WebSocket 连接正常

- [ ] **5. 测试验证**
  - [ ] 机器人已加入测试群
  - [ ] 日志显示 WebSocket 已连接
  - [ ] 能接收并响应 @机器人 消息

## 🐛 常见问题

### 1. 平台显示"应用未建立长连接"

**症状**：
- 机器人日志显示 `connected`
- 但飞书平台显示"应用未建立长连接"
- 无法保存事件订阅配置

**解决方案**：
1. 确认机器人进程正在运行
2. 检查日志是否显示 `connected to wss://msg-frontier.feishu.cn`
3. 等待 2-3 分钟，刷新页面重试
4. 或直接继续配置事件订阅，可能是平台检测延迟

### 2. 机器人收不到消息

**检查清单**：
1. [ ] 事件是否添加：im.message.receive_v1
2. [ ] 权限是否开启：im:message
3. [ ] 机器人是否在群里
4. [ ] 是否 @ 了机器人（群聊必需）
5. [ ] 查看机器人日志是否有错误

**调试命令**：
```bash
# 检查机器人进程
ps aux | grep "bot/main"

# 查看日志
tail -50 /tmp/feishu-bot.log

# 搜索错误
grep -i "error" /tmp/feishu-bot.log
```

### 3. CardKit 更新失败

**可能原因**：
- 超过 10 QPS 限流
- token 过期
- card_id 或 element_id 错误
- 权限不足

**解决方案**：
- 检查日志中的错误信息
- 验证 `cardkit:card:write` 权限
- 确保代码中有适当的限流（100ms 间隔）

### 4. 版本发布后才能生效

**重要**：以下配置修改需要发布版本才能生效：
- 权限的添加/修改
- 应用能力变更
- 某些安全设置

**发布版本步骤**：
1. 点击"创建版本"
2. 填写版本号和说明
3. 提交审核（内部应用自动通过）
4. 等待版本生效

## 📚 相关文档

- [飞书开放平台文档](https://open.feishu.cn/document)
- [CardKit 2.0 指南](https://open.feishu.cn/document/common-capabilities/message-card/card-components)
- [事件订阅指南](https://open.feishu.cn/document/server-docs/event-subscription-guide)
- [飞书 Go SDK](https://github.com/larksuite/oapi-sdk-go)

## 🔄 配置模板

### 快速复制配置

**应用信息**：
```yaml
应用类型: 企业自建应用
App ID: cli_a9dc39c0c2b8dbc8
App Secret: [从飞书平台获取]
```

**必需权限**：
```yaml
permissions:
  - im:message
  - im:message.group_at_msg
  - im:chat
  - cardkit:card:write
```

**事件订阅**：
```yaml
subscription_mode: 长连接
events:
  - im.message.receive_v1
```

---

**最后更新**：2026-01-02
**维护者**：Claude Code
**适用于**：飞书开放平台企业自建应用
